...
void LinkedList ::GoNext() { (this->now) = &(*this->now)->next; }

void LinkedList ::GoFirst() { (this->now) = &(this->start); }
...
int main() {
  ...
  while (menu != 0) {
    ...
    printf(
        "  Menu - (1) Add (2) Edit (3) Delete (4) Find (5) Show (0) Edit : ");
    scanf("%d", &menu);
  } // end while
  // writefile( &listA ) ;
  return 0;
